...
def export_txt(report: AnalysisReport, output_path: Path) -> Path:
    ...
    if report.mining_quality:
        ...
        lines += _section("MINING QUALITY INDICATORS")
        _fields = [
    ...
    if report.intrinsic_value:
        ...
        rows = [
            ...
            _iv_row("EV/Resource Implied", iv.ev_resource_implied_price, None),
        ]
        for r in rows:
            if r is not None:
                lines.append(r)
    ...
    if c.screening_checklist:
        lines.append("")
        lines.append("  Mining Screening Checklist:")
        lines.append("  " + "-" * 40)
    ...
    lines.append(f"  Lynx Basic Materials (Lince Investor Suite)")
    lines.append("=" * W)
