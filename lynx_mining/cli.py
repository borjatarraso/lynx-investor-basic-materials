"""Command-line interface for lynx-mining."""
...
from lynx_mining import __author__, __author_email__, __license__, __version__, __year__, SUITE_LABEL


def build_parser():
    parser = argparse.ArgumentParser(
        prog="lynx-mining",
        description="Lynx Basic Materials Analysis — Junior Mining & Basic Materials fundamental analysis tool.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  lynx-mining -p UUUU                          US uranium producer\n"
            "  lynx-mining -p DML.TO                         TSX mining stock\n"
            "  lynx-mining -p OCO.V                          TSXV junior miner\n"
            "  lynx-mining -p FUU.V --refresh                Force fresh data\n"
            '  lynx-mining -p "Denison Mines"                Search by name\n'
            "  lynx-mining -p -i                             Interactive mode\n"
            "  lynx-mining -t UUUU                           Testing mode\n"
        ),
    )
...
def run_cli():
    ...
    if "--b2m" in sys.argv:
        ...
        from lynx_mining.easter import rich_rocket, rich_fortune
        c = Console()
        rich_rocket(c)
        rich_fortune(c)
        return
    ...
    if "--explain" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from rich.table import Table
        from lynx_mining.metrics.explanations import get_explanation, list_metrics
    ...
    from lynx_mining.core.storage import set_mode, is_testing
    ...
    if args.interactive:
        from lynx_mining.interactive import run_interactive; run_interactive(); return
    if args.tui:
        from lynx_mining.tui.app import run_tui; run_tui(); return
    if args.gui:
        from lynx_mining.gui.app import run_gui; run_gui(args); return
    if args.search:
    ...
    from lynx_mining.display import display_report_stage
    try:
        ...
        if args.export:
            from pathlib import Path; from lynx_mining.export import ExportFormat, export_report
...
def _cmd_list_cache(con):
    from rich.table import Table; from lynx_mining.core.storage import list_cached_tickers, get_mode
