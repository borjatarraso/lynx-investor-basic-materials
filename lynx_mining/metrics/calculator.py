"""Mining-specialized metrics calculation engine.
...
from lynx_mining.models import (
    ...
    MiningQualityIndicators, ProfitabilityMetrics, ShareStructure,
...
def calc_mining_quality(
    ...
    if revenues and stage == CompanyStage.PRODUCER:
        if len(revenues) >= 2 and revenues[0] > revenues[1]:
            m.revenue_predictability = "Producer with growing revenue"
            score += 15
        else:
            m.revenue_predictability = "Producer with revenue"
            score += 10
    elif stage in (CompanyStage.EXPLORER, CompanyStage.DEVELOPER):
        m.revenue_predictability = f"{stage.value} — pre-revenue stage"
        score += 5
    elif stage == CompanyStage.GRASSROOTS:
        m.revenue_predictability = "Grassroots explorer — no revenue expected"
        score += 2
    elif stage == CompanyStage.ROYALTY:
        m.revenue_predictability = "Royalty/streaming revenue" if revenues else "Early-stage royalty"
...
def calc_intrinsic_value(
    ...
    if stage == CompanyStage.PRODUCER:
        if tier in (CompanyTier.LARGE, CompanyTier.MID, CompanyTier.MEGA):
            ...
            iv.secondary_method = "EV/EBITDA Comps"
        else:
            iv.primary_method = "P/NAV"
        ...
        iv.primary_method = "EV/Resource"
        iv.secondary_method = "Asset-Based (Tangible Book)"
    elif stage == CompanyStage.ROYALTY:
        iv.primary_method = "DCF"
        iv.secondary_method = "P/NAV"
    else:
...
def calc_market_intelligence(
    ...
    # ── 8. Mining disclaimers ────────────────────────────────────────
    disclaimers: list[str] = [
        "Mining investments carry high risk. Junior explorers have >90% failure rate.",
        "Resource estimates are geological opinions, not guaranteed economic deposits.",
        "Commodity prices are cyclical — current margins may not persist.",
    ]
    if stage in (CompanyStage.GRASSROOTS, CompanyStage.EXPLORER):
        disclaimers.append(
            "This company is pre-revenue. Valuation is speculative and based "
            "on exploration potential, not cash flows."
        )
    if stage == CompanyStage.DEVELOPER:
        disclaimers.append(
            "Construction cost overruns of 20-50% above feasibility estimates "
