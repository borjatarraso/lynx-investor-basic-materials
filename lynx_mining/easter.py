...
[bold green]      | |  --/ V \\--   |  |      [/][dim]Basic Materials Analysis[/]
[bold green]       \\ \\    ___    / /
        \\ \\---------/ /
         \\___________/[/]
"""
...
[bold yellow]
   ...
   (d  b--b  b)      [dim]Junior Mining Research[/dim]
     |__|__|
[/bold yellow]
"""
...
FORTUNE_QUOTES = [
    ...
    '"In mining, the best deposits are found by the best geologists, not the best promoters." \u2014 Rick Rule',
...
def tk_fireworks(root) -> None:
    ...
    def _animate():
        ...
        particles = alive
        if step[0] == 1:
            for i, msg in enumerate(["\u2605  lynx-mining  \u2605", "Basic Materials Analysis", "\U0001f680 To the Moon! \U0001f680"]):
...
def tk_rainbow_title(root, count: int = 20) -> None:
    ...
    def _step(remaining, idx):
        if remaining <= 0:
            ...
            return
        try: root.title(f"{random.choice(sparkles)} {colors[idx % len(colors)]} lynx-mining {colors[idx % len(colors)]} {random.choice(sparkles)}")
        except tk.TclError: return
