"""Mining-focused report synthesis engine."""

from __future__ import annotations

import math

from lynx_mining.models import AnalysisConclusion, AnalysisReport, CompanyStage, CompanyTier, JurisdictionTier
...
def generate_conclusion(report: AnalysisReport) -> AnalysisConclusion:
    ...
    quality_score = _safe(report.mining_quality.quality_score) if report.mining_quality else 0
    ...
    c.category_scores = {"valuation": round(val_score, 1), "profitability": round(prof_score, 1),
                         ...
                         "mining_quality": round(quality_score, 1)}
    ...
    c.screening_checklist = _mining_screening(report)
    return c
...
def _mining_screening(r: AnalysisReport) -> dict:
    checks = {}
...
def _build_summaries(r: AnalysisReport) -> dict[str, str]:
    ...
    summaries["mining_quality"] = (r.mining_quality.competitive_position or "N/A") if r.mining_quality else "N/A"
    return summaries
...
def _find_strengths(r: AnalysisReport) -> list[str]:
    ...
    if r.profile.jurisdiction_tier == JurisdictionTier.TIER_1:
        strengths.append("Tier 1 mining jurisdiction")
    return strengths[:6]
...
def _find_risks(r: AnalysisReport) -> list[str]:
    ...
    if r.profile.jurisdiction_tier == JurisdictionTier.TIER_3:
        risks.append("Tier 3 jurisdiction — high risk")
    if r.profile.stage == CompanyStage.GRASSROOTS:
        risks.append("Grassroots stage — binary outcome risk")
    return risks[:6]
...
def _stage_note(stage: CompanyStage) -> str:
    return {
        CompanyStage.PRODUCER: "Producer: traditional metrics + AISC/production. EV/EBITDA and P/NAV primary.",
        CompanyStage.DEVELOPER: "Developer: P/NAV from economic studies. Cash runway critical. Cost overruns (20-50%) common.",
        CompanyStage.EXPLORER: "Explorer: EV/resource and P/NAV from PEA. Cash runway, dilution, geology are key. Pre-revenue.",
        CompanyStage.GRASSROOTS: "Grassroots: call option on discovery. Cash backing, management, land position. Binary risk.",
