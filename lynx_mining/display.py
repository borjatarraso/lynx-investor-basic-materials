"""Rich console display for mining analysis reports — stage and tier aware.
...
def _a_divy(val):
    if _isna(val):
        return "[dim]No dividend — typical for juniors[/]"
    if val == 0:
...
def _a_roa(val):
    ...
    if val < 0.08:
        return "[green]Good ROA for capital-intensive mining[/]"
...
def display_full_report(report):
    ...
    if report.mining_quality:
        _display_mining_quality(report)
...
def display_report_stage(stage, report):
    ...
    handlers = {
        ...
        "mining_quality": lambda: (
            _progressive_stages_seen.add("mining_quality"),
            _display_mining_quality(report),
        ),
...
def _display_header(report):
    ...
    console.print(Panel(
        header,
        title="[bold]LYNX Basic Materials Analysis[/]",
        border_style="blue",
    ))
...
def _display_mining_quality(report):
    """Mining quality assessment — uses relevance markers via get_relevance with category='mining_quality'."""
    m = report.mining_quality
    if m is None:
        return
    tier, stage = report.profile.tier, report.profile.stage
...
def _add_quality_row(table, label, value, relevance):
    """Add a row to the mining quality table with relevance-based styling."""
    if relevance == Relevance.IRRELEVANT:
        return
    style, prefix = _STYLE[relevance]
    sl = f"{prefix}[{style}]{label}[/]" if style else f"{prefix}{label}"
...
def _display_screening(c):
    ...
    t = Table(title="Mining Screening Checklist", border_style="cyan")
...
def _display_market_intelligence(report):
    ...
    if mi.disclaimers:
        console.print(Panel(
            ...
            title="[dim]Mining Investment Disclaimers[/]",
            border_style="dim",
        ))
