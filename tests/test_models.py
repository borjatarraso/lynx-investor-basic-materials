...
from lynx_mining.models import (
    ...
    MiningQualityIndicators, ShareStructure, MarketIntelligence,
...
class TestClassifyStage:
    def test_producer_with_revenue(self):
        assert classify_stage("gold mining operations with AISC", 50_000_000) == CompanyStage.PRODUCER
    ...
    def test_industry_hint(self):
        assert classify_stage("company overview", 0, {"industry": "Gold"}) == CompanyStage.EXPLORER
...
class TestClassifyCommodity:
    ...
    def test_gold(self):
        assert classify_commodity("gold mining au", "Gold") == Commodity.GOLD

    def test_copper(self):
        assert classify_commodity("copper cu porphyry", "Other Industrial Metals") == Commodity.COPPER

    def test_lithium(self):
        assert classify_commodity("lithium spodumene project", None) == Commodity.LITHIUM

    def test_silver(self):
        assert classify_commodity("primary silver producer ag", "Silver") == Commodity.SILVER
    ...
    def test_no_false_silver_in_saskatchewan(self):
...
class TestClassifyJurisdiction:
    ...
    def test_peru_tier2(self):
        assert classify_jurisdiction("Peru") == JurisdictionTier.TIER_2
