...
from lynx_mining.metrics.calculator import (
    ...
    calc_efficiency, calc_share_structure, calc_mining_quality,
...
class TestCalcMiningQuality:
    def test_quality_score_range(self, sample_info, sample_statements):
        ...
        m = calc_mining_quality(p, g, s, ss, sample_statements, sample_info,
                                CompanyTier.MICRO, CompanyStage.EXPLORER)
        assert 0 <= m.quality_score <= 100
        assert m.competitive_position is not None

    def test_empty_inputs(self):
        m = calc_mining_quality(ProfitabilityMetrics(), GrowthMetrics(), SolvencyMetrics(),
