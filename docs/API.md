...
│   └── sector_insights.py # Mining industry insights
├── export/
...
### Analysis (`lynx_mining.core.analyzer`)
...
Run a complete fundamental analysis for a mining or basic materials company.
...
```python
from lynx_mining.core.analyzer import run_full_analysis
...
`"solvency"` | `"growth"` | `"share_structure"` | `"mining_quality"` |
...
```python
from lynx_mining.core.analyzer import run_progressive_analysis
...
### Conclusion (`lynx_mining.core.conclusion`)
...
For example, solvency and mining quality are weighted much more heavily for
...
- `category_scores` -- dict with scores for `valuation`, `profitability`, `solvency`, `growth`, `mining_quality`.
...
## Data Models (`lynx_mining.models`)
...
| `Commodity` | `GOLD`, `SILVER`, `COPPER`, `URANIUM`, `LITHIUM`, `NICKEL`, `ZINC`, `RARE_EARTHS`, `OTHER` |
...
| `sector` | `Optional[str]` | Sector (e.g. `"Basic Materials"`). |
...
| `stage` | `CompanyStage` | Mining development stage (default `GRASSROOTS`). |
...
Traditional and mining-specific valuation ratios.
...
Margins and returns, plus mining-specific cost metrics.
...
| `mining_quality` | `Optional[MiningQualityIndicators]` | Composite quality score. |
...
## Classification Helpers (`lynx_mining.models`)
...
Classify mining development stage by keyword matching against the company
description.  Companies with revenue > $10M and royalty/streaming keywords
are classified as `ROYALTY`.  Falls back to industry-based heuristics from
...
## Metrics Calculator (`lynx_mining.metrics.calculator`)
...
|---|---|---|
| `calc_valuation(info, statements, tier, stage)` | `ValuationMetrics` | P/E, P/B, EV/EBITDA, P/FCF, cash-to-market-cap, tangible book, NCAV. Mining-specific: EV/resource, P/NAV. |
| `calc_profitability(info, statements, tier, stage)` | `ProfitabilityMetrics` | ROE, ROA, ROIC, gross/operating/net/FCF/EBITDA margins. Mining-specific: AISC. |
...
| `calc_mining_quality(profitability, growth, solvency, share_structure, statements, info, tier, stage)` | `MiningQualityIndicators` | Composite quality score (0-100) based on insider ownership, financial position, dilution risk, jurisdiction, and more. |
...
## Relevance System (`lynx_mining.metrics.relevance`)
...
development stage is the primary analytical axis for mining companies.
...
| `category` | `str` | One of `"valuation"`, `"profitability"`, `"solvency"`, `"growth"`, `"mining_quality"`, `"share_structure"`. |
| `stage` | `CompanyStage` | Mining development stage. |
...
## Storage (`lynx_mining.core.storage`)
...
## Ticker Resolution (`lynx_mining.core.ticker`)
...
```python
from lynx_mining.export import ExportFormat
```

---

## Sector Insights (`lynx_mining.metrics.sector_insights`)
...
Available sectors: `"Basic Materials"`, `"Energy"`.
...
Available industries: `"Gold"`, `"Other Industrial Metals & Mining"`,
`"Uranium"`, `"Other Precious Metals & Mining"`, `"Specialty Mining & Metals"`.
...
from lynx_mining.models import CompanyTier, CompanyStage, Relevance
