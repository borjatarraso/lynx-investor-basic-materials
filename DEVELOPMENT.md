...
The application follows the same architecture as `lynx-fundamental` with mining-specific adaptations.
...
calculator.py: calc_valuation/profitability/solvency/growth/...
    ↓
calculator.py: calc_share_structure + calc_mining_quality
    ↓
...
1. **Stage > Tier**: Mining company development stage is the primary analysis axis, not market cap tier. The relevance system prioritizes stage overrides.
...
5. **Mining Disclaimers**: Every analysis includes stage-specific risk disclosures to ensure responsible use.
...
python -c "import py_compile, glob; [py_compile.compile(f, doraise=True) for f in glob.glob('lynx_mining/**/*.py', recursive=True)]"
```
