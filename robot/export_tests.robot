...
Export HTML From Minimal Report
    ...
    When I Run Python Code "from lynx_mining.models import AnalysisReport, CompanyProfile, CompanyTier, CompanyStage, ValuationMetrics, SolvencyMetrics; from lynx_mining.export import export_report, ExportFormat; from pathlib import Path; import tempfile; p = CompanyProfile(ticker='TEST', name='Test Mining Corp', sector='Basic Materials', industry='Gold', country='Canada', market_cap=100000000); p.tier = CompanyTier.MICRO; p.stage = CompanyStage.EXPLORER; r = AnalysisReport(profile=p, valuation=ValuationMetrics(pb_ratio=0.8, cash_to_market_cap=0.35), solvency=SolvencyMetrics(cash_runway_years=3.0, total_cash=35000000)); f = tempfile.NamedTemporaryFile(suffix='.html', delete=False); path = export_report(r, ExportFormat.HTML, Path(f.name)); content = path.read_text(); assert '#fff' in content or '#ffffff' in content, 'Missing white background'; assert 'word-wrap' in content, 'Missing word-wrap'; assert 'Test Mining Corp' in content; assert len(content) > 1000; print('OK', len(content), 'bytes')"
    Then The Exit Code Should Be 0
    Then The Output Should Contain "OK"
